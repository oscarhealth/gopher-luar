...
func Test_func_immutable_result(t *testing.T) {
	...
	getPerson := func() *StructTestPerson {
		return &StructTestPerson{Name: "Tim"}
	}
...
func Test_func_transparentptr_result(t *testing.T) {
	// When using reflect options on a function, those options should be applied
	// to output values from the function. In this case, the output should have
	...
	val := "hello"
	newB := func() *TestTransparentPtrAccessB {
		return &TestTransparentPtrAccessB{Str: &val}
	}
